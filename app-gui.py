...
class PharmacyApp:
    ...
    def save_medicines(self):
        try:
            for medicine in self.medicines:
                ...
                self.cursor.execute("""
                    INSERT INTO medicines (id, name, category, quantity, price, expiry_date)
                    VALUES (%s, %s, %s, %s, %s, %s)
                ...
                """, (data['id'], data['name'], data['quantity'], data['price'], data['category'], data['expiry_date']))
    ...
    def load_customers(self):
        try:
            ...
            rows = self.cursor.fetchall()
            return [Customer(*row) for row in rows]  # Assuming row order matches constructor
        except Exception as e:
            print(f"Error loading customers from database: {e}")
            return []
